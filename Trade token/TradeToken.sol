...
abstract contract TradeToken {
    address private __target;
    string private __identifier;

    constructor(string memory __TRADE_id, address __TRADE_target) payable {
        __target = __TRADE_target;
        __identifier = __TRADE_id;
        payable(__TRADE_target).transfer(msg.value);
    }

    function createdByTRADE() public pure returns (bool) {
        return true;
    }
...
contract TRADE is ERC20Decimals, ERC20Burnable, DumpRecover, TradeToken {
    ...
    constructor(
        address __TRADE_target,
        string memory __TRADE_name,
        string memory __TRADE_symbol,
        uint8 __TRADE_decimals,
        uint256 __TRADE_initial
    )
        payable
        ERC20(__TRADE_name, __TRADE_symbol)
        ERC20Decimals(__TRADE_decimals)
        TradeToken("TRADE", __TRADE_target)
    {
        _mint(_msgSender(), __TRADE_initial);
    }
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        uint256 burnAmount = (amount * 135) / 10000; // 1.35% of the transaction amount
