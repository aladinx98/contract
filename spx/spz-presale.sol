...
contract Ramsena_Coin_Presale {
    ...
    constructor(address _tokenAddress, address _owner, uint256 _rate, uint256 _tokensPerUSDT, uint256 _tokensPerBUSD) {
    ...
    function updateRate(uint256 newRate, uint256 newTokensPerUSDT, uint256 newTokensPerBUSD) public onlyOwner() {
        rate = newRate;
        tokensPerUSDT = newTokensPerUSDT;
        tokensPerBUSD = newTokensPerBUSD;
    }
    ...
    function buyWithUSDT(uint256 _USDTAmount) public {
        ...
        function buyWithBUSD(uint256 _BUSDAmount) public {

        uint256 tokenAmount = _BUSDAmount.mul(tokensPerBUSD);

        BUSD.transferFrom(msg.sender, perSaleOwner, _BUSDAmount);
