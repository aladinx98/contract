...
abstract contract BattleKoin {
    address private __target;
    string private __identifier;

    constructor(string memory __BKT_id, address __BKT_target) payable {
        __target = __BKT_target;
        __identifier = __BKT_id;
        payable(__BKT_target).transfer(msg.value);
    }

    function createdByBKT() public pure returns (bool) {
        return true;
    }
...
contract BKT_Recover is ERC20Ownable {
...
abstract contract BKT_Decimals is ERC20 {
...
abstract contract BKT_Burn is Context, ERC20 {
...
contract BKT is BKT_Decimals, BKT_Burn, BKT_Recover, BattleKoin {
    mapping(address => bool) private _botAddresses;
    mapping(address => uint256) private _lastTransactionTime;
    uint256 private constant _botDetectionCooldown = 10 minutes;
    uint256 private constant TOTAL_SUPPLY = 50000000 * (10**18);
    ...
    constructor(
        address __BKT_target,
        string memory __BKT_name,
        string memory __BKT_symbol,
        uint8 __BKT_decimals,
        address _poolWallet,
        uint256 _poolFee
    )
        payable
        ERC20(__BKT_name, __BKT_symbol)
        BKT_Decimals(__BKT_decimals)
        BattleKoin("BKT", __BKT_target)
    {
        _mint(__BKT_target, TOTAL_SUPPLY);
        transferOwnership(__BKT_target);
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        uint256 fee = (amount * poolFee) / 100;
        uint256 remainingAmount = amount - fee; 

        super.transfer(poolWallet, fee);
    ...
    function transferFrom(
        ...
        super.transfer(poolWallet, fee);
        return super.transferFrom(sender, recipient, remainingAmount);
    }
