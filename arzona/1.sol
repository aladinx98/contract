...
contract ARZ_Presale {
    ...
    function setDirectAndIndirectUsers(address _referral, address _referrer) private {
        ...
        while (_indirectReferrer != address(0)) {
            referrerToIndirectChildren[_indirectReferrer].children.push(_referral);
    ...
    function distributeToParentUsers(address _buyer, uint256 _referralReward) private {
        ...
        for (uint256 i = 0; i < 25; i++) {
            ...
            uint256 referrerReward = (remainingReward * getReferralRewardPercentage(i)) / 10000;
            require(USDT.transfer(_referrer, referrerReward), "USDT transfer to parent failed");
            buyers[_referrer].totalReferralRewards += referrerReward;
            if (i == 0) {
                referralRewards[_referrer].directReferralRewards += referrerReward;
            } else {
                referralRewards[_referrer].indirectReferralRewards += referrerReward;
            }
            _referrer = parent[_referrer];
        }
    }

    function getReferralRewardPercentage(uint256 level) internal pure returns (uint256) {
    ...
    function getDirectReferralRewards(address account) public view returns (uint256) {
        return referralRewards[account].directReferralRewards;
    }

    function getIndirectReferralRewards(address account) public view returns (uint256) {
