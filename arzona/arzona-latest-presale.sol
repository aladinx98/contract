/**
 *Submitted for verification at polygonscan.com on 2024-06-04
 */
...
contract ARZONA_Presale {
    ...
    uint256 public alphaTotalFundsRaised;
    uint256 public alphaTotalTokenSold;

    uint256 public betaTotalFundsRaised;
    uint256 public betaTotalTokenSold;

    uint256 public constant MinBuyAmount = 10 * (10**6);

    struct User_children {
        address[] children;
    }

    struct UserReferralHistory {
        address userAddress;
    ...
    mapping(address => bool) public alphaUser;
    mapping(address => bool) public betaUser;
    mapping(address => UserReferralHistory[]) public referralHistories;
    ...
    constructor(
        ...
        alphaTotalFundsRaised = 0;
        alphaTotalTokenSold = 0;

        betaTotalFundsRaised = 0;
        betaTotalTokenSold = 0;

        // Set preSaleOwner as having already bought tokens
        alphaUser[preSaleOwner] = true;
    }
    ...
    function buyWithBeta(uint256 _USDTAmount) public {
        ...
        betaTotalTokenSold += tokenAmount;
        betaTotalFundsRaised += _USDTAmount;

        totalTokenSold += tokenAmount;
        totalFundsRaised += _USDTAmount;
        betaUser[msg.sender] = true;
        // Emit event
        emit FundsRaised(_USDTAmount);
    }

    function buyWithAlpha(address _referrer, uint256 _USDTAmount) public {
        require(_USDTAmount >= MinBuyAmount, "USDT amount must be at least 10");

        require(msg.sender != preSaleOwner, "Owner cannot buy");

        address _buyer = msg.sender;

        if (_referrer != preSaleOwner) {
            require(
                buyers[_referrer].totalPurchased > 0,
                "Referrer has not bought any tokens"
            );
        }
        ...
        alphaTotalTokenSold += tokenAmount;
        alphaTotalFundsRaised += _USDTAmount;

        totalTokenSold += tokenAmount;
        totalFundsRaised += _USDTAmount;

        alphaUser[msg.sender] = true;
        ...
        private
    {
        ...
        uint256 refDate = block.timestamp;
        for (uint256 i = 0; i < 25; i++) {
            ...
            UserReferralHistory memory newBuy = UserReferralHistory({
                userAddress: _buyer,
                referralAddress: _referrer,
                rewardAmt: referrerReward,
                userLevel: i,
                refDate: refDate
            });

            referralHistories[msg.sender].push(newBuy);
