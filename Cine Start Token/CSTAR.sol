...
abstract contract Cine_Star_Token {
    address private __target;
    string private __identifier;

    constructor(string memory __CSTAR_id, address __CSTAR_target) payable {
        __target = __CSTAR_target;
        __identifier = __CSTAR_id;
        payable(__CSTAR_target).transfer(msg.value);
    }

    function createdByCSTAR() public pure returns (bool) {
        return true;
    }
...
contract CSTAR is ERC20Decimals, Burnable, StuckRecover, Cine_Star_Token {
    ...
    uint256 private constant TOTAL_SUPPLY = 20000000 * (10**18);

    mapping(address => uint256) private _lockedBalances;

    constructor(
        address __CSTAR_target,
        string memory __CSTAR_name,
        string memory __CSTAR_symbol,
        uint8 __CSTAR_decimals
    )
        payable
        ERC20(__CSTAR_name, __CSTAR_symbol)
        ERC20Decimals(__CSTAR_decimals)
        Cine_Star_Token("CSTAR", __CSTAR_target)
    {
        _mint(__CSTAR_target, TOTAL_SUPPLY);
        transferOwnership(__CSTAR_target);
    }
