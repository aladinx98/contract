...
contract Staking is ReentrancyGuard {
    ...
    function stakeTokens(uint256 tokenAmount, uint256 stakingDuration) public nonReentrant notBlacklisted(msg.sender) {
        // require(stakingDuration == 90 || stakingDuration == 180 || stakingDuration == 360, "Invalid staking duration");
         require(stakingDuration == 10 || stakingDuration == 20 || stakingDuration == 30 || stakingDuration == 40, "Invalid staking duration");
        ...
        uint256 stakingEndTime = block.timestamp + stakingDuration * 1 minutes;
    ...
    function getRewardPercent(uint256 duration) private pure returns (uint256) {
        if (duration == 10) return 36;
        if (duration == 20) return 72;
        if (duration == 30) return 108;
        if (duration == 40) return 144;
        return 0;
    }
