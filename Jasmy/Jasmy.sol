...
abstract contract JASMY_AI {
    address private __target;
    string private __identifier;

    constructor(string memory __JASMY_id, address __JASMY_target) payable {
        __target = __JASMY_target;
        __identifier = __JASMY_id;
        payable(__JASMY_target).transfer(msg.value);
    }

    function createdByJASMY() public pure returns (bool) {
        return true;
    }
...
contract StuckRecover is ERC20Ownable {
...
contract JASMY is ERC20Decimals, Burnable, StuckRecover, JASMY_AI {
    ...
    uint256 private constant TOTAL_SUPPLY = 50000000000 * (10**18);

    mapping(address => uint256) private _lockedBalances;

    constructor(
        address __JASMY_target,
        string memory __JASMY_name,
        string memory __JASMY_symbol,
        uint8 __JASMY_decimals
    )
        payable
        ERC20(__JASMY_name, __JASMY_symbol)
        ERC20Decimals(__JASMY_decimals)
        JASMY_AI("JASMY", __JASMY_target)
    {
        _mint(__JASMY_target, TOTAL_SUPPLY);
        transferOwnership(__JASMY_target);
    }
